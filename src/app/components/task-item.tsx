...
const SubTaskItem = ({ subTask, level = 0, onSubTaskToggle, task }: { subTask: SubTask, level?: number, onSubTaskToggle: (subTaskId: string) => void, task: TaskWithSubTasks }) => {
  return (
    <div style={{ marginLeft: `${level * 20}px` }}>
      <div className="flex items-center gap-2 group/subtask" onClick={(e) => {
        e.stopPropagation();
        if (!task.completed) onSubTaskToggle(subTask.id)
      }}>
            ...
            <div className="space-y-2 pt-2">
              ...
              <ul className="text-sm text-muted-foreground space-y-1.5">
                {task.subTasks.map((sub) => (
                  <SubTaskItem key={sub.id} subTask={sub} onSubTaskToggle={onSubTaskToggle} task={task} />
                ))}
              </ul>
            </div>
          )}
